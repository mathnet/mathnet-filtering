    ...
    public class SquareArchitectures : GenericArchitectureFactory
    {
            ...
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Integer"),
                ...
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<IntegerValue,IntegerValue>(delegate(IntegerValue value) { return value.Multiply(value); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Rational"),
                RationalValueCategory.IsRationalValueMember,
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<RationalValue,RationalValue>(delegate(RationalValue value) { return value.Multiply(value); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Real"),
                RealValueCategory.IsRealValueMember,
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<RealValue,RealValue>(delegate(RealValue value) { return value.Multiply(value); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<ComplexValue, ComplexValue>(ComplexValue.Square, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
