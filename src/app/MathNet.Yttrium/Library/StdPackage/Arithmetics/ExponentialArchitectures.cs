    ...
    public class ExponentialArchitectures : GenericArchitectureFactory
    {
            ...
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Real"),
                ...
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<RealValue,RealValue>(delegate(RealValue value) { return value.Exponential(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<ComplexValue, ComplexValue>(ComplexValue.Exponential, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
