...
namespace MathNet.Symbolics.StdPackage.Properties
{
    ...
    /// </summary>
    public sealed class PositiveIntegerSetProperty : Property
    {
        private static readonly MathIdentifier _propertyId = new MathIdentifier("PositiveIntegerSet", "Std");
        ...
        public override bool StillValidAfterEvent(Signal signal)
        {
            if(signal.Value == null)
                return false;
            return IntegerValue.Converter.CanConvertLosslessFrom(signal.Value) && IntegerValue.ConvertFrom(signal.Value).Value >= 0;
        }
