    ...
    public class CosineArchitectures : GenericArchitectureFactory
    {
            ...
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Real"),
                ...
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<RealValue, RealValue>(delegate(RealValue value) { return value.Cosine(); }, RealValue.ConvertFrom, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new Process[] { new GenericStdParallelProcess<ComplexValue, ComplexValue>(delegate(ComplexValue value) { return value.Cosine(); }, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
