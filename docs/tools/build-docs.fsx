...
let info =
  [ "project-name", "Math.NET Filtering"
    ...
    "project-github", "http://github.com/mathnet/mathnet-filtering"
    "project-nuget", "http://nuget.com/packages/MathNet.Filtering" ]
...
let prepareReleaseNotes() =
    for (fileName, docName, title) in releaseNotesDocs do
        String.concat Environment.NewLine
          [ "# " + title
            "[Math.NET Filtering](ReleaseNotes.html)"
            ""
    ...
    for lib in referenceBinaries do
        MetadataFormat.Generate
            (bin @@ lib, output @@ "reference", layoutRoots, parameters = ("root", root) :: info,
             sourceRepo = "https://github.com/mathnet/mathnet-filtering/tree/master/src", sourceFolder = @"..\..\src",
             publicOnly = true)
