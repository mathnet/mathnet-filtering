...
let provideFsLoader includes path =
    ...
    let assemblies = [ "MathNet.Filtering.dll"; "MathNet.Filtering.FSharp.dll" ]
...
let provideFsIfSharpLoader path =
    let fullScript = ReadFile "src/FSharp/MathNet.Filtering.IfSharp.fsx" |> Array.ofSeq
    let startIndex = fullScript |> Seq.findIndex (fun s -> s.Contains "***MathNet.Filtering.IfSharp.fsx***")
    ReplaceFile (path </> "MathNet.Filtering.IfSharp.fsx") (fullScript .[startIndex + 1 ..] |> toLines)
...
let publishDocs (release:Release) =
    ...
    Git.Commit.Commit repo (sprintf "Filtering: %s docs update" release.PackageVersion)
...
let publishApi (release:Release) =
    ...
    Git.Staging.StageAll repo
    Git.Commit.Commit repo (sprintf "Filtering: %s api update" release.PackageVersion)
    Git.Branches.pushBranch repo "origin" "master"
...
let publishArchive zipOutPath nugetOutPath (bundles:Bundle list) =
    let archivePath = (environVarOrFail "MathNetReleaseArchive") </> "Math.NET Filtering"
